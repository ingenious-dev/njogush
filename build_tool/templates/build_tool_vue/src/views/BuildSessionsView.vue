      ...
      <div class="bg-white shadow sm:overflow-hidden sm:rounded-lg">
        <div class="divide-y divide-gray-200">
          <div class="px-4 py-5 sm:px-6 flex">
            <div class="flex-auto">
              <h2 id="notes-title" class="text-lg font-medium text-gray-900">Build Sessions</h2>
            </div>
            <!-- <button
              type="button"
          ...
          <tbody class="divide-y divide-gray-200 bg-white">
            <tr v-for="item in buildSessions" :key="item.id">
              ...
              <td class="relative whitespace-nowrap py-2 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                <a href="#" class="text-indigo-600 hover:text-indigo-900" @click="openSideBar(item.id)"
                    ...
                    <div class="h-0 flex-1 overflow-y-auto">
                      <div class="bg-indigo-700 py-6 px-4 sm:px-6">
                        <div class="flex items-center justify-between">
                          <DialogTitle class="text-lg font-medium text-white">{{ newOrEdit }} Build Session</DialogTitle>
                      ...
                      <div class="flex flex-1 flex-col justify-between">
                        <div class="divide-y divide-gray-200 px-4 sm:px-6">
                          <div class="space-y-6 pt-6 pb-5">
                            <!-- <div>
                              <label for="project-name" class="block text-sm font-medium text-gray-900">Build Session name</label>
                              <div class="mt-1">
                                <input
                                type="text"
                                name="project-name"
              ...
              <DialogPanel class="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                ...
                <div class="sm:flex sm:items-start">
                  ...
                  <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    <DialogTitle as="h3" class="text-lg font-medium leading-6 text-gray-900">Delete build session</DialogTitle>
                    <div class="mt-2">
                      <p class="text-sm text-gray-500">Are you sure you want to delete the build session? <br> This action cannot be undone.</p>
                    </div>
                  </div>
                </div>
                <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                  <button type="button" class="inline-flex w-full justify-center rounded-md border border-transparent bg-red-600 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 sm:ml-3 sm:w-auto sm:text-sm" @click="deleteBuildSession()">Delete</button>
...
export default {
  ...
  methods: {
    openSideBar(id) {
      this.selectedId = id;
      if(id) {
        this.setForm(this.currentBuildSession)
      } else {
        this.setForm({})
      }
      this.open = true
    ...
    async save() {
      ...
      if(this.selectedId) {
        await this.$store.dispatch('updateBuildSession', {id: this.selectedId, data: data});
      } else {
        await this.$store.dispatch('createBuildSession', data);
        this.closeSideBar();
        this.setForm({})
      }
    },
    async deleteBuildSession() {
      await this.$store.dispatch('deleteBuildSession', {id: this.selectedId});
  ...
  mounted() {
    ...
    this.$store.dispatch('fetchBuildSessions', {project: this.projectId, limit: 1000});
  }
}
</script>
